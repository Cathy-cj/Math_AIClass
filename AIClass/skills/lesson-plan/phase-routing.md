...
3. `teachingStages[]` 第一项固定为 `read-problem / 审题环节`，覆盖读图、标条件、明确所求与关键（含问句陷阱点名）；对应步骤使用 `group: 1`。第二项固定为 `entry-point / 从哪入手`，展开 `entryPoint.strategy`（范式级，非操作步骤；见 teaching-design.md §2）。
...
| practice | afterExample（`afterPlanId` 指向例题） | 无；入口后先调用手写板 | `opening.practice` |
