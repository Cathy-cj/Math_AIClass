...
fill-lesson-plan 阶段必读。与 [figure-animation.md](figure-animation.md) 配合，适用于**所有题型**批量生产。
...
- 从 outline 原样复制 `problemBrief`
- `known`、`ask` 必填，`key` 可选
- 禁止加入 `knowledge`，禁止生成 `card: "readStem"`
- 组件嵌入第一项“审题环节”的槽位，不单独出卡；读图/审题步骤使用 `group: 1`
- 每个审题动画步骤写 `problemBrief` 快照：`known` 是当前已显示的已知条数，随后逐步打开 `ask`、`key`
- 快照必须单调累积，“求”先于“关键”；禁止第一步直接显示全部内容
...
- 左图同步：连线、设点、垂线等新对象在对应的那一拍出现（见 figure-animation.md）
...
- **审题环节（group 1）禁止互动**（硬契约见 [reference.md](../lesson-plan/reference.md)「审题环节禁止互动」）：只写 `problemBrief` 快照与口播，不设问/答步
...
## guidance 与 group（所有 left-right 题必做）

- 有 `guidanceChain` 的 left-right Plan 顶层必须写 `"guidanceLayout": "interleaved"`
- 可见正文 group 从 1 开始，与 `guidanceChain[]` 一一对应；`group: 0` 仅用于开场
- 正文必须完整覆盖 `1..guidanceChain.length`，不允许缺号或越界
- `region: "right"` 的 push 必须进入本 group 的 `.cc-guide-slot`，不得漂在环节卡外
- **每个环节槽位不得空白**（desc 已废弃，槽内没有兜底文字）：除 group 1（`problemBrief` 自动嵌入兜底）外，每个 group 至少落一张右栏卡——
  - 「从哪入手」环节：把 outline `entryPoint.strategy` 压成**范式级**短语卡（4–12 字，如「化整为零」；如 `replaceKey: "{id}:strategy"`）。操作步骤（拆分、算占比、列式顺序）不进此卡，留给推导 group
...
- **答步 / 过渡口播步**可以无 `push`、无 `figure`（仅口播）；运行时不得插入 action 占位文字
- 有动画：`figure.state` + **`figure.note`（必填，写清高亮/标注/染色）**
...
契约**唯一权威是 [reference.md](../lesson-plan/reference.md)「agent.description ＝ TTS 逐字稿」**：纯中文汉字（数字/算式写读法，几何点名可留大写字母）、符号一律写读法（加/减/乘/除/三角形/角/度/根号，单位写汉字）、禁括号屏幕说明（动画细节归 `figure.note` / `figure.actions[]`，制作备注归 `moduleNote`）、只用中文标点。fill 阶段另注意：
