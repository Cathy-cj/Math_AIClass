...
大纲 = **一次完整备课**：不只是解析，而是把「这道题为什么值得学、学生会卡在哪、从哪切入、每轮推导干什么、在哪抛互动题、最后带走什么」全部设计好。用户审的是**备课质量**。**禁止**在此阶段写 plan.json、action 名或 figure state。
...
1. 定 `id`、`moduleType`、`figureTemplate`（**不定 action 前缀**）；**md 输入时** `id` 按 [naming-from-md.md](../lesson-plan/naming-from-md.md)（单题 = md 基名，多题 = 基名 + `-ex1/-pr1/…`）
2. **先独立解题、标难点**：只看题干把题完整做一遍，记下思维链里每个「必须想到才行」的动作（设参、辅助线、换视角、逆用公式……）——这就是难点清单，供 `entryPoint` 与各段 `difficulty` 印证；【分析】【详解】用来校对结果，不用来代替自己的推演。然后写 `analysis`、`analysisModel`（题干信息、逻辑链、列式顺序）、`solution[]`
   ...
   - 另写 `problemBrief`：`known`、`ask` 必填；`key` 可选
...
10. **practice** 写 `lessonContext.slot: "afterExample"` 与 `afterPlanId`，指向前一例题；练题先让学生独立手写，再沿同一教学骨架讲解，**不写** `quickQASkeleton`。
...
12. 在**对话里**输出摘要（定位、切入点、环节链、互动、列式）供事后查阅；**自动**进入下一阶段（有图 → figure-space-clarify，无图 → fill-lesson-plan）
13. 若已挂到某门课：刷新看板 `npm run pipeline:board -- <courseId>`（`outlineOk` 由看板自动推断）
14. 若本题有图 / `figureTemplate`：进入 **figure-space-clarify**（唯一人工停点），再写 `_*-figure.js`

## 禁止

- 不写 `stepSkeleton`、不写 `action` / `figureState` / `pushKind`
...
有图时下一步进入 figure-space-clarify，等人明确 **图形 OK**；无图时自动进入 fill-lesson-plan。
