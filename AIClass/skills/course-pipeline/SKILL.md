...
description: >-
  课件制作看板：查看一门课各题处在大纲/图形/讲法/编排/生成的哪一步，写入门禁并推荐下一步。
...
同一 `problemId` 内 AI 可链式推进；**仅 `needsFigure && !figureOk` 时等人审**。
...
| 类型 | 字段 | 行为 |
|------|------|------|
| **人工** | `figureOk` | 用户明确说 **图形 OK** / **按这个画** 后写入 |
| **自动** | `outlineOk` | 刷新看板时：`outline.json` 存在且 `outlineStatus: approved` |
| **自动** | `planOk` | 刷新看板时：`plan.json` 存在，且 outline 已 OK、（无图或 figureOk） |
...
- 用户只说「嗯 / 好」且尚未看过 figure-preview → **不要**写 `figureOk`
...
| figure-space | [figure-space-clarify](../figure-space-clarify/SKILL.md) |
