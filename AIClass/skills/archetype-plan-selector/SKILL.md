...
**本 skill 不参与 AIClass 课件生产主流程。** 正式课程必须走 [production-flow](../production-flow/SKILL.md)：`lesson-outline` → 门禁 →（有图则 figure-space-clarify）→ `fill-lesson-plan` → codegen。`math_syllabus/lesson/{id}/outline.json` 与 `plan.json` 是唯一内容真源。

本 skill 仅用于**外部**规划任务：读 `engine/references/**` 获取 archetype 结构启发，**不得**直接产出并落地 plan.json、不得绕过 **图形 OK** 门禁。
