...
| [`../skills/`](../skills/) | **SOP 真源**（大纲/讲法/图形/codegen/编排） | 调规范时改这里 |
...
flowchart LR
  ...
  outline -->|有图| figureGate
  outline -->|无图| plan
  figureGate -->|fill-lesson-plan| plan
  plan --> codegen
```

| 阶段 | 真源（唯一产物） | 推进方式 | 粒度 | 不含 |
|------|------|----------|------|------|
| ① 大纲 | `outline.json` | `plan:check` 通过后自动（auto `outlineOk`） | 大环节、解题思路、列式 | action、figure、逐步口播 |
| 图形 | `figure-spec.json` + preview | **唯一人工停点**：**图形 OK** | 空间关系、坐标 | — |
| ② 讲法 | `plan.json` | `plan:check` 通过后自动（auto `planOk`） | 逐步 action、左图动画、右栏 push、口播 | — |

**顺序固定**：大纲 →（有图则图形 OK）→ plan → codegen。改某步口播 → 改 `plan.json` 对应 step → `npm run plan:check`。
...
4. **有图题**：看 `figure-preview.html`，回复 **「图形 OK」**（唯一人工停点）  
5. 自动 **填充 plan** → 生成 `plan.json`  
6. `plan:check` 通过后自动 codegen / 预览  
7. 改稿：说「第 N 步口播改成…」或自己改 JSON 后跑 `plan:check`  
...
| [codegen-lesson](../skills/codegen-lesson/SKILL.md) | 落地课件 | `courses/` 模块 + figure |
