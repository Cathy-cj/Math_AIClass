 ...
 *   node tools/pipeline-board.mjs <courseId> --gate <problemId> figureOk
...
function emptyGates() {
  return { outlineOk: false, figureOk: false, planOk: false }
}
...
function inferGates(row, disk) {
  ...
  const needsFigure = row.needsFigure !== false

  if (disk.hasOutline && disk.outline?.outlineStatus === 'approved') {
    gates.outlineOk = true
  }

  if (disk.hasPlan && gates.outlineOk && (!needsFigure || gates.figureOk)) {
    gates.planOk = true
  }
...
function scanProblem(authoringRoot, courseDir, course, row) {
  ...
  const hasFigureSpace =
    fs.existsSync(path.join(dir, 'figure-space.md')) ||
    fs.existsSync(path.join(dir, 'figure-spec.json'))
  const modules = course.authoredModules || []
  const hasFigureModule = modules.some(
    (m) => typeof m === 'string' && m.includes(id) && m.includes('-figure')
  )
  ...
  let figureCol = '—'
  if (needsFigure) {
    if (!hasFigureSpace && !hasFigureModule) figureCol = '缺'
    else figureCol = gates.figureOk ? 'OK' : 'draft'
  }
  ...
  else if (needsFigure && !gates.figureOk) {
    next = hasFigureSpace || hasFigureModule
      ? '审图 / figure-space → 图形 OK'
      : 'figure-space-clarify → 图形 OK'
...
function renderMarkdown(courseId, pipeline, rows) {
  ...
  const lines = [
    ...
    '',
    '| problemId | 大纲 | 图形 | 讲法 | 已编排 | 已生成 | 预览 | 下一步 |',
    '|-----------|------|------|------|--------|--------|------|--------|'
  ]
  for (const r of rows) {
    lines.push(
      `| ${r.problemId} | ${r.outlineCol} | ${r.figureCol} | ${r.planCol} | ${r.scheduledCol} | ${r.generatedCol} | ${r.previewCol} | ${r.next} |`
    )
  }
  if (!rows.length) {
...
function printTable(courseId, rows) {
  console.log(`\n制作看板 — ${courseId}\n`)
  console.log(
    ['problemId', '大纲', '图形', '讲法', '编排', '生成', '预览', '下一步']
      .map((h) => h.padEnd(12))
      .join('')
  )
  for (const r of rows) {
    console.log(
      [
        ...
        .map((c, i) => String(c).padEnd(i === 0 || i === 7 ? 28 : 8))
        .join('')
    )
  }
  console.log('')
...
function applyGate(pipeline, problemId, gateName) {
  const allowed = new Set(['outlineOk', 'figureOk', 'planOk'])
...
function main() {
  ...
  if (!courseId) {
    console.error(
      'Usage: node tools/pipeline-board.mjs <courseId> [--sync] [--gate <problemId> figureOk] [--complete-preview <problemId>]'
    )
    process.exit(1)
  }
