  ...
  for (const step of plan.steps) {
    ...
    for (const block of step.push || []) {
      if (block.card === 'readStem') {
        throw new Error(`readStem cards are replaced by problemBrief: ${plan.id}/${step.id}`)
      }
...
function generatedModule(plan, problem, displayIndex) {
  ...
  const sideEffects = steps.slice(1).map((step) => {
    ...
    if (step.guidanceDesc) item.guidanceDesc = step.guidanceDesc
    return item
  })
...
function actionCatalog(plan, module) {
  ...
  for (const effect of module.sideEffects || []) {
    if (!entries.some((item) => item.name === effect.action)) {
      const item = { name: effect.action, params: [], description: effect.description || '' }
      if (effect.photoAnswer === true) entries.splice(1, 0, item)
...
function buildLessonMeta(course) {
  const base = {
    ...
    defaults: {
      layoutParams: { edgePad: 32, textMaxWidth: 560, figureWidth: 480, gap: 28 },
...
function generateCourse(courseId) {
  ...
  const figureModules = collectFigureModulePaths(course.dir)
  const authoredModules = course.config.authoredModules || []
  const manifestModules = [...figureModules, ...authoredModules, ...modules].filter(
...
function exportCourse(courseId, options = {}) {
  ...
  try {
    ...
    const locked = err && (err.code === 'EBUSY' || err.code === 'EPERM' || /busy|locked/i.test(String(err.message)))
    if (!locked) throw err
    ...
    removeDirectory(temp)
    console.warn(`Export overlay: ${finalDir} was in use; synced in place (close debug/preview tabs for a clean replace).`)
  }
  if (options.zip) {
