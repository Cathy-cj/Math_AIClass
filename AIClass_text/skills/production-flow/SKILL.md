...
description: >-
  纯文字题课件制作全流程导航：看板 → 大纲 → 讲法 → 编排 → 落地 → 生成预览。
...
本仓只制作纯文字题；同一题内可连续自动推进，不设图形人工门禁。一次只服务一门课（一个 `courseId`）。
...
标准教学单元：**例题（含顶部快问快答）→ 练题**。快问快答绑定例题 plan，不单独登记为 problem。

制作时按题逐一完成：例题依次走完大纲 → 讲法 → 落地 → check/generate/preview 验收；**例题预览验收通过后自动开始配对练题**。不得先把同一单元的例题与练题同时写完大纲或 plan。
