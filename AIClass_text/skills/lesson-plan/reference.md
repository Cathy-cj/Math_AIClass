...
| 禁止 | `action`、`pushKind`、逐步 `beat` | `figureTemplate`、`figure` |
...
- `problemBrief`（outline）：`known`、`ask` 必填，`key` 可选——**仅备课分析，不上屏**；课件审题用 push 的 `section`「已知/求」（见 [text-only-marks.md](text-only-marks.md)）。
...
**不得出现**：`stepSkeleton`、`stepBudget`（已废弃——步数预算只会限制展开，深度由 `difficulty` 决定，见 teaching-design「难点展开」）、`guidanceChain`（已并入 `teachingStages[]`）、`guidanceDesc` / `desc`（已废弃，课件不再展示环节副标题）、`action`、`figureState`、`figureTemplate`。
...
每步含 `id`、`action`、`phase`（题内中文 title）、`group`、`agent`、`push`、`moduleNote`。仅开场使用 `group: 0`；审题步骤属于第一环节并使用 `group: 1`，后续环节按 `teachingStages[]` 顺序连续编号。
...
- 审题上屏用 `section`「已知/求」逐步揭示，并用 `stemClass` 同步点亮题干；口播可点名陷阱、知识预告、难度播报
...
**每个环节槽位必须有内容**：group 1 由 `section`「已知/求」逐步填满；其余每个 group 至少落一张右栏卡；「从哪入手」落 `entryPoint.strategy` 范式级短语卡（4–12 字，非操作步骤），「提取已知」落数据卡（拆步细则见 [step-splitting.md](../fill-lesson-plan/step-splitting.md)）。

禁止生成 `card: "readStem"` 或在 push 中展示“知识点”。审题已知/求用 `section` + `tagTone` 上屏（传参见 [text-only-marks.md](text-only-marks.md)），逐步追加，禁止一步甩出全部。
