...
Agent 与用户以 **`.md` 文件**作为题目输入时的唯一命名权威。引擎 `validSlug` 校验：`/^[a-z0-9][a-z0-9-]{1,47}$/`（2–48 字符小写 ASCII slug，允许数字开头，禁止连字符开头）。
...
多题后缀可压缩课次中的连字符（如 `6-21` → `621`），但 **courseId 保持与 md 文件名一致**。参考计算仓现有课 `courses/sum-6-21-5star/`。
