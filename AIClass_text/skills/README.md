...
| Directory | Role |
|-----------|------|
| [production-flow/](production-flow/SKILL.md) | 纯文字题全流程导航（每题自动推进） |
...
| [lesson-plan/](lesson-plan/) | Shared schema / phrase-bank；text-only 高亮传参见 [text-only-marks.md](lesson-plan/text-only-marks.md) |
...
本仓只承载纯文字题，不设图形门禁。`outlineOk` / `planOk` / `previewOk` 会在检查/预览通过后自动写入。
...
| AI 只产出 json（不导 md）、纯文字题内自动链式推进 | [production-flow/](production-flow/SKILL.md) |
