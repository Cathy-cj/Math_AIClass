...
function validatePlanSemantics(plan) {
  ...
  if (plan.moduleType !== 'knowledge') {
    ...
    // text-only：审题上屏用 section 标签「已知/求」；禁止 plan/step.problemBrief
...
function generatedStepOptions(step, planId) {
  const options = {}
  for (const key of ['scroll', 'stemClass', 'guidanceSub']) {
...
function actionCatalog(plan, module) {
  const entries = (module.containers || []).flatMap((container) =>
  ...
  for (const sideEffect of module.sideEffects || []) {
    if (!entries.some((item) => item.name === sideEffect.action)) {
      entries.push({
        name: sideEffect.action,
        params: [],
...
function buildLessonMeta(course) {
  const base = {
    ...
    defaults: {
      layoutParams: { edgePad: 32, textMaxWidth: 'none', gap: 28 },
