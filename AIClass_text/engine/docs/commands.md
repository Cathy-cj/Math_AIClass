...
`pipeline:board` / `pipeline.json` 已删除；制作进度由 agent 直接读 `_output_/{grade}/<courseId>/<problemId>/` 判断（outline/plan 的存在性与状态）。
...
- 课专属调试页：`_output_/{grade}/<id>/debug/index.html`（`course:new` / `lesson:generate` 同步）
