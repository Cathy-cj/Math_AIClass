...
`course_fill`、`course_choice`、`voice`；所有 `value` 均为字符串。

每个 `moduleType: "practice"` 在生成时自动追加
`{actionPrefix}_作答_拍照` action。父页面在练习题入口 action 已执行后派发它，课件会
显示“作答结果 / 拍照上传”区域。用户点击按钮时，课件上报：
...
派发拍照 action 的练习题作答区域，不触发判题或教学步骤推进；挂载成功后课件上报
