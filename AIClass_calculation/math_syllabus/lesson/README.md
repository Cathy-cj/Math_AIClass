...
| 文件 | 阶段 | 内容 | 你怎么审 |
|------|------|------|----------|
| **outline.json** | ① | `teachingStages[]` 阶段链、`analysisModel` 解题模型、答案 | 一般不看 JSON |
| **outline.md** | ① | 按阶段编号的解题思路、标准答案 | 阶段对不对、列式是否与【分析】一致 |
| **plan.json** | ② | `steps[]` 逐步：action、口播、push（top-split 三区域 + `.calc-*`） | 一般改 JSON 后 export |
| **plan.md** | ② | 步骤总览 + 逐步口播 / 要点 / 详解 | 口播、互动、揭示节奏 |
