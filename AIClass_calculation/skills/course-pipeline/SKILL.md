...
description: >-
  纯计算题课件制作看板：查看一门课各题处在大纲/讲法/编排/生成的哪一步，并推荐下一步。
...
同一 `problemId` 内 AI 可链式推进；纯计算题**不设** `figureOk` / 人工图形门禁。
...
验收标准见 [calc-engine-layout.md](../lesson-plan/calc-engine-layout.md)「previewOk 验收清单」（`data-layout=top-split`、右栏钉、长式缩字/断行规则、滚到底分式不裁切）。未通过不得 `--complete-preview`。
