...
import { listFigureSpecFiles, listOutlineOnlyJsonFiles, listPlanJsonFiles } from './lesson-paths.mjs'

const root = path.dirname(path.dirname(fileURLToPath(import.meta.url)))
const phraseBankPath = path.join(root, '..', '..', 'skills', 'plan', 'common', 'phrase-bank.json')
...
/** 屏幕极简：口播讲理，屏幕留证（见 skills/outline/common.md） */
function checkScreenTextBrevity(plan, label) {
  ...
  if (brief) {
    ...
    if (known.some((item) => String(item).length > 18) ||
        String(brief.ask || '').length > 32 ||
        String(brief.key || '').length > 24) {
      console.warn(
        `[screen] ${label}: problemBrief 须为短语级（known 每条 ≤18 字，ask ≤32，key ≤24）`
      )
      warnings++
    }
  }
...
/** desc 废弃后环节槽内无兜底文字：除 group 1（problemBrief 兜底）外，每个 group 至少一张右栏卡 */
function checkGuideSlotCoverage(plan, label) {
  ...
  for (const step of plan.steps || []) {
    ...
    const count = (step.push || []).filter((block) => block.region === 'right').length
    rightCards.set(group, (rightCards.get(group) || 0) + count)
  }
  for (let group = 2; group <= guidanceCount; group++) {
    if (!(rightCards.get(group) > 0)) {
      const title = plan.guidanceChain[group - 1]?.title || ''
      console.warn(
        `[guide-slot] ${label}: group ${group}（${title}）没有任何右栏卡，环节槽位会空白——从哪入手落 strategy 短语卡，提取已知落数据卡`
      )
      warnings++
    }
  }
...
function checkReadProblemNoInteraction(plan, label) {
  ...
  for (const step of plan.steps || []) {
    ...
    if (step.userResponse != null || badPush) {
      console.warn(
        `[read-problem] ${label} ${step.id}: 审题环节（group 1）禁止互动问/答——只写 problemBrief 快照与口播`
      )
      warnings++
    }
  }
...
function checkPlanStructure(plan, label) {
  ...
  if (plan.layout === 'left-right' && guidanceCount &&
      plan.guidanceLayout !== 'interleaved') {
    console.warn(
      `[guidance] ${label}: left-right 多环节必须使用 guidanceLayout="interleaved"`
    )
    warnings++
  }
  ...
  if (plan.moduleType !== 'knowledge' &&
      (!brief || !known?.some((item) => String(item || '').trim()) ||
       !String(brief.ask || '').trim())) {
    console.warn(`[problemBrief] ${label}: 已知和求必须生成`)
    warnings++
  }
  if (brief && Object.prototype.hasOwnProperty.call(brief, 'knowledge')) {
    console.warn(`[problemBrief] ${label}: 知识点只供内部分析，禁止展示`)
    warnings++
  }
  if (brief) {
    const revealSteps = (plan.steps || []).filter((step) => step.problemBrief)
    const snapshots = revealSteps.map((step) => step.problemBrief.known || 0)
    const knownCount = (Array.isArray(brief.known) ? brief.known : [brief.known])
    ...
    if (!revealSteps.length ||
        ...
        snapshots.some((count, index) => index > 0 && count < snapshots[index - 1]) ||
        (knownCount > 1 && new Set(snapshots).size < 2) ||
        (finalReveal?.known || 0) < knownCount ||
        finalReveal?.ask !== true ||
        (brief.key && (finalReveal?.key !== true || keyIndex <= askIndex))) {
      console.warn(`[problemBrief] ${label}: 已知/求/关键必须在审题动画中按快照渐进出现`)
      warnings++
    }
  }
  ...
  for (const step of plan.steps || []) {
    for (const block of step.push || []) {
      if (block.card === 'readStem') {
        console.warn(`[readStem] ${label}: 已由 problemBrief 取代，禁止生成 readStem 卡`)
        warnings++
      }
    }
  }
...
function checkOutlineStructure(filePath, label) {
  ...
  if (outlineBrief) {
    ...
    if (known.some((item) => String(item).length > 18) ||
      ...
      console.warn(
        `[outline] ${label}: problemBrief 会上屏，须为短语级（known 每条 ≤18 字，ask ≤32，key ≤24）`
      )
      warnings++
    }
  }
...
/**
 * figure-spec.json 基本校验：顶层必需字段 + states/actions 目标引用一致性。
 * spec 写错（对象名对不上）会在渲染/运行时才暴露，这里提前拦截。
 */
function checkFigureSpec(lessonDir, label) {
  ...
  try {
    ...
    console.warn(`[figure-spec] ${label}: figure-spec.json 不是合法 JSON（${e.message}）`)
    return 1
  }
  const required = ['id', 'dimension', 'engine', 'figureTemplate', 'status', 'board', 'points', 'states', 'actions']
  for (const key of required) {
    if (!(key in spec)) {
      console.warn(`[figure-spec] ${label}: 缺少顶层字段 ${key}`)
      warnings++
    }
  }
  const known = new Set(['parabola'])
  if (spec.points && typeof spec.points === 'object') {
    for (const k of Object.keys(spec.points)) known.add(k)
  }
  for (const item of [...(spec.segments || []), ...(spec.lines || [])]) {
  ...
  for (const [stateName, state] of Object.entries(spec.states || {})) {
    for (const t of (state.availableTargets || [])) {
      if (!known.has(t)) {
        console.warn(`[figure-spec] ${label}: states.${stateName}.availableTargets 引用未知目标「${t}」`)
        warnings++
      }
    }
  }
  for (const [actionName, action] of Object.entries(spec.actions || {})) {
    for (const t of (action.targets || [])) {
      if (!known.has(t)) {
        console.warn(`[figure-spec] ${label}: actions.${actionName}.targets 引用未知目标「${t}」`)
        warnings++
      }
    }
  }
...
function main() {
  ...
  for (const filePath of files) {
    ...
    const label = path.join(plan.id, 'plan.json')
    ...
    warnings += checkFigureLabels(plan, label)
    warnings += checkVolumeFigureSemantics(plan, label)
    warnings += checkFigureActions(plan, label)
  ...
  for (const outlinePath of outlineOnlyFiles) {
    ...
    const label = path.join(lessonId, 'outline.json')
    warnings += checkOutlineStructure(outlinePath, label)
  }

  const specFiles = listFigureSpecFiles()
  for (const specPath of specFiles) {
    const lessonId = path.basename(path.dirname(specPath))
    const label = path.join(lessonId, 'figure-spec.json')
    warnings += checkFigureSpec(path.dirname(specPath), label)
  }
  ...
  if (warnings === 0) {
    const outlineNote = outlineOnlyFiles.length
      ...
      : ''
    const specNote = specFiles.length
      ? `, ${specFiles.length} figure-spec`
      : ''
    console.log('plan:check OK —', files.length, 'plan(s)' + outlineNote + specNote)
  } else {
